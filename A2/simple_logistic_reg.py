...
with sess.as_default():
	tf_X_train=tf.constant(X_train[:train_subset,:],tf.float32)
	tf_X_test=tf.constant(X_test,tf.float32)
	tf_X_val=tf.constant(X_val,tf.float32)
	tf_y_train=tf.constant(y_train[:train_subset],tf.float32)
	...
	for iteration in range(num_iterations):
		_,l,predictions=sess.run([trainer,loss,train_prediction])
		if (iteration%100==0):
			print("Loss at step %d: %f" %(iteration,l))
